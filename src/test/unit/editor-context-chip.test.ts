...
import { chipLabel, lineSpan } from '../../webview/components/editor-context-chip';
...
suite('lineSpan', () => {
  test('no selection is the empty string, not a bare colon', () => {
    assert.strictEqual(lineSpan({ path: 'a.ts', languageId: 'typescript' }), '');
  });

  test('a single-line range collapses to one number, colon-prefixed', () => {
    assert.strictEqual(lineSpan({
      path: 'a.ts',
      languageId: 'typescript',
      selection: { ranges: [{ startLine: 7, endLine: 7, text: 'x' }], truncated: false },
    }), ':7');
  ...
  test('a multi-line range keeps both ends', () => {
    assert.strictEqual(lineSpan({
      path: 'a.ts',
      languageId: 'typescript',
      selection: { ranges: [{ startLine: 60, endLine: 73, text: 'x' }], truncated: false },
  ...
  test('extra ranges beyond the first are counted, not listed', () => {
    assert.strictEqual(lineSpan({
      path: 'a.ts',
      languageId: 'typescript',
      selection: {
