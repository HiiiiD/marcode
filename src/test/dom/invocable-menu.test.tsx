...
suite("invocable menu", () => {
  test("typing / opens the menu and a name filters it", async () => {
    renderWithStore(<Composer pane={pane(ENTRIES)} model={NO_EFFORT} models={[]} />);
  ...
  test("a space closes the menu and Enter sends again", async () => {
    ...
    assert.deepStrictEqual(
      sends.map((m) => (m as { text: string }).text),
      ["/loop 5m"],
    );
  });
  ...
  test("arrows move the active option and Enter inserts it", async () => {
    ...
    assert.strictEqual(
      document.getElementById(activeId ?? "")?.textContent?.includes("superpowers:writing-plans"),
      true,
    );
  ...
  test("the focused textarea points at the highlighted row", async () => {
    ...
    assert.ok(document.getElementById(second ?? "")?.textContent?.includes("superpowers:writing-plans"));
  });
  ...
  test("duplicate-named entries both render and the second one scrolls into view", async () => {
    ...
    renderWithStore(<Composer pane={pane(dup)} model={NO_EFFORT} models={[]} />);
    const box = screen.getByLabelText("Message") as HTMLTextAreaElement;
  ...
  test("mousedown on the overflow line does not blur the textarea", async () => {
    ...
    renderWithStore(<Composer pane={pane(many)} model={NO_EFFORT} models={[]} />);
    const box = screen.getByLabelText("Message");
