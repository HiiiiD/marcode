...
suite('fleet diff surface', () => {
  ...
  test('opening the surface asks the host for a diff', () => {
    renderReview();
    sendFromHost(hydrate());
    assert.strictEqual(posted().some((m) => m.t === 'request-fleet-diff'), true);
  });

  test('a changed file is listed under the session that claimed it', () => {
    renderReview();
    sendFromHost(hydrate());
  ...
  test('the base is named, so a head-only diff cannot pass for a full one', () => {
    renderReview();
    sendFromHost(hydrate());
    sendFromHost({ t: 'fleet-diff', trees: [TREE] });
  ...
  test('an empty answer reads as an answer, not as nothing asked', () => {
    renderReview();
    sendFromHost(hydrate());
  ...
  test('an empty answer does not claim the trees are clean', () => {
  ...
  test('a failed read says so, instead of reading forever', () => {
    renderReview();
    sendFromHost(hydrate());
  ...
  test('every tree and every session group is a heading', () => {
    renderReview();
    sendFromHost(hydrate());
    sendFromHost({ t: 'fleet-diff', trees: [TREE] });
  ...
  test('the header count explains the rows a shared file adds', () => {
